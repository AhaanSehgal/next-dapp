...
const { authUrl, walletUrl, socketUrl, environment, apiUrl } = config
...
const TriaHome = () => {
    ...
    // const { disconnectAsync, disconnect: wagmiDisconnect } = useWagmiDisconnect()
    ...
    const callSignMsg = async (chainName: string = "POLYGON") => {
        const { message } = {
            // chainName: "MUMBAI",
            message: `hello it worked on ${chainName}`,
        };
        // if (localStorage.getItem("wagmi.connected") == "true") {
        //   const { WalletController } = await import("@tria-sdk/utils")
        //   const wallet = new WalletController({
        ...
        <div>
            {
                account ?
                    <>
                        ...
                        <div>{account?.triaName}</div>
                        <div>{account?.evm?.address}</div>
                    </>
                    : null
            }
            <button
            ...
            <button
                className=" top-2 left-2 px-2 py-2 bg-green-500 text-white rounded-md"
                onClick={() => callSignMsg("ETH")}
            >
                Sign Message ETH
            </button>
            <button
                className=" top-2 left-2 px-2 py-2 bg-green-500 text-white rounded-md"
                onClick={() => callSignMsg("MUMBAI")}
            >
                Sign Message Mumbai
            </button>
            <button
                className=" top-2 left-2 px-2 py-2 bg-green-500 text-white rounded-md"
                onClick={() => callSignMsg("FUSE")}
            >
                Sign Message Fuse
            </button>
            <button
